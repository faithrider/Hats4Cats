...
"""
It's official: Rekognition doesn't *do* cat faces.
TODO: Train neural net to recognize cat-face feature coordinates

import base64
import sys
from os.path import abspath as getpath
...
def main():
    ...
    for i in range(1, 15):
        image = boto3.resource('s3', region_name='us-east-2').Object('846466456841-in', 'image' + str(i).zfill(3) + '.jpg')
        query = RekognitionImage.from_bucket(image, rekognition_client)
        pprint(query)
        faces = query.detect_faces()
        for face in faces:
            pprint(face.to_dict())
        # show_bounding_boxes(query.image['Bytes'], [[face.bounding_box for face in faces]], 'aqua')
